...
# FastAPI
from fastapi import FastAPI, HTTPException, UploadFile, File, Form, Request, status, Depends, Response, BackgroundTasks, WebSocket, WebSocketDisconnect
...
# ═══════════════════════════════════════════════════════════════
# WEBSOCKET LOG STREAMING (Cross-platform: Windows + Linux)
# ═══════════════════════════════════════════════════════════════
import threading
import queue
...
class WebSocketLogManager:
    ...
    Works on both Windows and Linux without creating new event loops.
    """
    def __init__(self):
        self.connections: list = []          # active WebSocket objects
        self.log_queue: queue.Queue = queue.Queue(maxsize=500)
        self._lock = threading.Lock()
        self._main_loop: asyncio.AbstractEventLoop = None  # set after app starts
    ...
    def broadcast(self, message: str):
        """Called from any thread - schedules send on the main asyncio loop."""
        if not self._main_loop or not self.connections:
            return
        asyncio.run_coroutine_threadsafe(self._send_all(message), self._main_loop)
...
class WebSocketHandler(logging.Handler):
    ...
    def emit(self, record):
        try:
            ws_manager.broadcast(self.format(record))
        except Exception:
            pass
...
OLLAMA_MODEL = os.getenv("OLLAMA_MODEL", "qwen2.5:14b")  # Recommended for your GPU
...
# Initialize Local Embeddings with GPU support
def init_embeddings():
...
# Initialize Whisper (lazy load on first use)
def init_whisper():
...
# ═══════════════════════════════════════════════════════════════
# FASTAPI LIFESPAN (Async Ollama Init)
# ═══════════════════════════════════════════════════════════════
@asynccontextmanager
async def lifespan(app: FastAPI):
    ...
    # ✅ Register the running event loop so ws_manager can schedule sends
...
class AsyncJobsCache:
    """Thread-safe async cache for scraped jobs"""
...
async def fetch_live_jobs_async(lang: str) -> str:
    """Async job fetching with concurrent detail scraping"""
...
def get_embedding(text: str):
    """Generate embeddings using LOCAL model only"""
    try:
        if local_embedder:
            if len(text) > 2000:
                text = text[:2000]
...
def robust_json_parse(raw_text: str) -> Optional[Dict]:
    """Robust JSON parser for Ollama output"""
    if not raw_text:
        return None
    ...
    try:
        ...
        response = await ollama_client.post("/api/generate", json={
            ...
            "options": {
                "temperature": 0.1,
                "num_predict": 2048,
                "num_ctx": 8192,  # Larger context for Qwen 14B
                "num_gpu": -1,
                "num_thread": 6,
                "repeat_penalty": 1.1,
                "top_p": 0.9,
...
async def process_user_query(question: str, language: str, history: List[ChatMessage]) -> tuple:
    ...
    # STEP 3: Vector search (skip for jobs)
    ...
    if is_irrelevant:
        if language == "ar":
            return "عذراً، هذا السؤال خارج نطاق تخصصي. أنا مساعد رقمي متخصص في خدمات الهيئة العامة للرقابة على الصادرات والواردات فقط.", [], []
        elif language == "en":
            return "Sorry, this question is outside my expertise. I'm a digital assistant specialized only in GOEIC services.", [], []
        else:
            return "Désolé, cette question est hors de mon domaine. Je suis spécialisé uniquement dans les services GOEIC.", [], []
    ...
    if not response_data:
        if language == "ar":
            return "عذراً، النظام مشغول حالياً. يرجى المحاولة مرة أخرى.", [], []
        elif language == "en":
            return "Sorry, the system is busy. Please try again.", [], []
        else:
            return "Désolé, le système est occupé. Veuillez réessayer.", [], []
    ...
    return answer, final_sources, suggestions
...
async def chat_endpoint(request: Request, req: ChatRequest):
    ...
    answer, sources, suggestions = await process_user_query(req.question, req.language, req.history)
...
async def voice_endpoint(request: Request, file: UploadFile = File(...), language: str = Form("ar")):
    ...
    # ✅ Cross-platform temp directory (works on Windows + Linux)
    import tempfile
    ...
    try:
        ...
        # Process query
        start_time = time.time()
        answer, sources, suggestions = await process_user_query(user_text, language, [])
...
async def websocket_logs(websocket: WebSocket):
    """Stream logs in real-time to admin dashboard"""
    ...
    try:
        ...
        logger.info(f"📡 WebSocket client disconnected (Remaining: {len(ws_manager.connections)})")
    except Exception as e:
...
async def trigger_website_scraping(background_tasks: BackgroundTasks, user: str = Depends(require_admin)):
    """
    Trigger background website scraping with duplicate prevention
    """
...
async def run_smart_scraper():
    """
    Full scraping pipeline triggered by the dashboard button.

    PIPELINE ORDER (all automatic, no manual steps):
      1. Connect to Weaviate
      2. smart_cleanup  — remove any leftover duplicates BEFORE scraping
      3. DuplicateDetector — load existing URL/hash index from DB
      4. SmartGOEICScraper — scrape & apply 5-filter pipeline
      5. index_to_weaviate — store only new/updated pages
      6. Final stats logged to browser via WebSocket
    """
    global scraping_status
    ...
    try:
        ...
        def scraper_worker():
            try:
                ...
                # ── STEP 2: Pre-scrape cleanup ───────────────────
                # Removes duplicate parents that may have accumulated from
                # previous interrupted runs — same logic as smart_cleanup.py
                ...
                # ── STEP 3: Load existing URL/hash index ─────────
                scraping_status.update({
                ...
                # ── STEP 4: Scrape with 5-filter pipeline ────────
                scraping_status.update({
                ...
                logger.info(
                    ...
                    f"skipped_db={s['skipped'] - s['skipped_junk'] - s['skipped_in_session_dup']} "
                    f"errors={s['errors']}"
                )
                scraping_status.update({
                    "message": (
                ...
                # ── STEP 5: Index new/updated pages ──────────────
...
async def trigger_excel_upload(
    ...
    """
    Upload Excel file and trigger document indexing
    """
...
async def run_smart_uploader(excel_path: str, base_dir: str):
    """Background task to run smart uploader - Windows Compatible"""
    global upload_status
    ...
    try:
        ...
        # Run in thread pool (cross-platform)
        running_loop = asyncio.get_running_loop()
        with ThreadPoolExecutor(max_workers=1) as executor:
